...
class CameraManager(object):
    ...
    def set_sensor(self, index, notify=True, force_respawn=False):
        ...
        if needs_respawn:
            ...
            self.sensor3 = self._parent.get_world().spawn_actor(
                ...
                attach_to=self._parent,
                attachment_type=self._camera_transforms[self.transform_index][1])
            print("qwerty",self.sensor1,index)
            print("qwerty",self.sensor2,index)
            print("qwerty",self.sensors[index],index)
            # We need to pass the lambda a weak reference to self to avoid
            # circular reference.
            weak_self = weakref.ref(self)
            self.sensor1.listen(lambda image: CameraManager._parse_image(weak_self, image))
            self.sensor2.listen(lambda image: CameraManager._parse_image1(weak_self, image))
            self.sensor3.listen(lambda image: CameraManager._parse_image2(weak_self, image))
        if notify:
    ...
    def _parse_image1(weak_self, image):
        ...
        if not self:
            return
        image.convert(self.sensors[3][1])
        array = np.frombuffer(image.raw_data, dtype=np.dtype("uint8"))
        array = np.reshape(array, (image.height, image.width, 4))
        array = array[:, :, :3]
        array = array[:, :, ::-1]
    ...
    def _parse_image2(weak_self, image):
        ...
        if not self:
            return
        if self.sensors[6][0].startswith('sensor.lidar'):
            points = np.frombuffer(image.raw_data, dtype=np.dtype('f4'))
            points = np.reshape(points, (int(points.shape[0] / 3), 3))
            lidar_data = np.array(points[:, :2])
            lidar_data *= min(self.hud.dim) / 100.0
